...
class Model:
    ...
    def __setup_placeholder(self):
        ...
        self.trade_data   = tf.placeholder("float", [None, in_size])
    ...
    def __setup_model(self):
        in_size = Model.INPUT_SIZE
        w1 = tf.Variable(tf.truncated_normal([in_size, Estimator.HIDDEN_UNIT_SIZE], stddev=0.1))
        ...
        w3 = tf.Variable(tf.truncated_normal([Estimator.HIDDEN_UNIT_SIZE2, Estimator.HIDDEN_UNIT_SIZE3], stddev=0.1))
        b3 = tf.Variable(tf.constant(0.1, shape=[Estimator.HIDDEN_UNIT_SIZE3]))
        ...
        w3 = tf.Variable(tf.truncated_normal([Estimator.HIDDEN_UNIT_SIZE3, 2], stddev=0.1))
        b3 = tf.Variable(tf.constant(0.1, shape=[2]))
        self.output = tf.nn.softmax(tf.matmul(h3_drop, w3) + b3)
...
class Trainer(Model):
    ...
    def __prepare_train(self, session):
        ...
        self.summary_writer = tf.summary.FileWriter('logs', graph_def=session.graph_def)
    ...
    def __print_status(self, session, i, data):
        ...
        print ('step {0} ,train_accuracy={1} ,test_accuracy={2} '.format(i, train_accuracy, test_accuracy))
